...
[Aqui está o firmware nativo para as impressoras AD5M, AD5MPro e AD5X](https://github.com/ghzserg/FF/releases/R)

## Como instalar o firmware nativo

1. Redefina a impressora para as configurações padrão
2. formate o flash USB para FAT/FAT16/FAT32
3. Coloque o arquivo na pasta raiz do USB Flash
4. Desligue a impressora
5. Insira a unidade flash USB na impressora
6. Ligue a impressora

## Firmware nativo do AD5X
...
- `config.tar` se você o fez anteriormente via `TAR_CONFIG`.
- Após a atualização, se você precisar reconstruir a instalação do MCU [Adventurer5M-3.1.9-2.2.3-20250807-Factory.tgz](https://github.com/ghzserg/FF/releases/download/R/Adventurer5M-3.1.9-2.2.3-20250807-Factory.tgz)

## AD5X-ENABLE-zmod.tgz

A ativação do zmod no AD5X após uma atualização de firmware nativo permite ativar o mod sem reinstalá-lo completamente

[AD5X-ENABLE-zmod.tgz](https://github.com/ghzserg/FF/releases/download/R/AD5X-ENABLE-zmod.tgz)

## Instalar o XY

Para aqueles que foram para a "casa" deslocada do telhado (estacionamento para o receptor de cocô). Extraia o conteúdo do arquivo para a raiz da unidade flash, insira a unidade flash e reinicie a impressora. A calibração será carregada. Primeiro, pressione Reset. Em seguida, defina a posição correta do cabeçote usando os botões de controle na tela. Fixe-a com o botão Set. Reinicie a impressora sem a unidade USB. Arquivo do suporte oficial do flashforge.
...
## E0011 no firmware nativo

[fix_e0011](/pt/Global/#fix_e0011)
...
## E0017 no firmware nativo

[fix_e0017](/pt/Global/#fix_e0017)
...
## 5X IFS
Arquivo de firmware do IFS

- [3.0.6](https://github.com/ghzserg/FF/releases/download/R/ifc_3.0.6.zip)

- Instruções de reparo do firmware IFS](/pt/AD5X/#11-recover-firmware-ifs)
...
## CONFIG
### Arquivos de configuração nativos
...
Remove links residuais da inicialização automática
...
## FIX_UPDATE_MCU
### Correção para atualizações constantes do MCU
...
## LOG
### Permite obter arquivos de log se o Klipper não estiver funcionando
