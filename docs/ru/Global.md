...
После первого запуска макроса, контроль будет автоматически включаться при печати.
...
_Фраза "Контроль не активен" обозначает, что в конкретный момент времени контроль не используется, но при печати активируется автоматически_

---

### GET_ZMOD_DATA

Получить значения глобальных параметров/флагов Z-Mod.
После выполнения макроса в консоли будут выведены данные, которые были сохранены ранее и применены в текущий момент

`Fluidd` -> `Макросы` -> `Основное` -> `ПАРАМЕТРЫ Z-Mod`
...
Сохранить глобальные параметры/флаги Z-Mod, применяются при каждой печати.
...
**Чтобы отредактировать нужный параметр перейдите `Fluidd` -> `Макросы` -> `Система` -> `СОХРАНИТЬ ПАРАМЕТРЫ Z-Mod`**, выбирайте параметр который хотите поменять, заполняйте его и нажмите `ОТПРАВИТЬ`. Смотрите, что выведет в консоли Fluidd.
...
Если Z-Mod успешно просканирует файл на наличие используемых инструментов, это значение будет переопределено, и будут показаны только те инструменты, которые используются в файле.
...
Включает сканирование gcode-файлов для определения используемых команд смены инструмента (T0, T1 и т. д.), а также цветов и материалов, назначенных им в слайсере:
...
Определяет, какие метки отображать в меню выбора цвета: начинающиеся с 0 (T0, T1 и т. д.) или начинающиеся с 1 (Color 1, Color 2 и т. д.). Это меняет только названия кнопок и служит исключительно для удобства: 
...
Определяет, следует ли пытаться автоматически сопоставлять команды смены инструмента (T0, T1 и т. д.) с физическим филаментом, загруженным в ваш IFS, при запуске печати. Если вы не включили тихий режим, меню выбора цвета все равно появится; эта настройка влияет только на выбор по умолчанию:

- 0 (выключено)
- 1 (включено)

Эта настройка также применяется к заданиям, запущенным в тихом режиме. Вы можете настроить ее так, чтобы печать прерывалась при возникновении определенных ошибок автоназначения: 

- 2 (прервать, если какой-либо материал не совпадает, но допускать несовпадение цветов)
- 30 (прервать при любых проблемах).
...
Использовать предочистку сопла в CLEAR_NOZZLE 0-нет, 1-да (0)
...
При каждой печати строить карту стола 0-нет, 1-да (0). *Для работы снятия карты стола с родного экрана необходимо, через меню экрана принтера перейти на вкладку "Настройки" -> "Иконка WiFi" -> "Сетевой режим" -> **включить ползунок** "Только локальные сети"*
...
Только Creator 5 / Creator 5 Pro

При каждой печати подбирать PA (Pressure Advance) 0-нет, 1-да (0). *Для работы Auto PA с родного экрана необходимо, через меню экрана принтера перейти на вкладку "Настройки" -> "Иконка WiFi" -> "Сетевой режим" -> **включить ползунок** "Только локальные сети"*

Пример: `SAVE_ZMOD_DATA PRINT_AUTOPA=1`
...
1.  **Исходные данные:** В памяти принтера хранится карта стола (AD5M/AD5X 25 точек, C5Pro 100 точек), полученная при последней процедуре выравнивания.
2.  **Подготовка:**

    *   Для AD5m/AD5X cопло прогревается до рабочей температуры, очищается о стол и остывает до температуры замера: [TEMP_TEST_MAX](/ru/Global/#temp_test_max) (по умолчанию 151), если целевая температура сопла выше 230°C, иначе [TEMP_TEST_MIN](/ru/Global/#temp_test_min) (по умолчанию 121).
    *   Для Creator 5 / Creator 5 Pro снимается активный экструдер
...
**Примечания для проверки без автоподбора:**
...
*   Проверка проходит при горячем сопле
...
Нижняя температура сопла для замера карты стола в режимах [MESH_TEST](/ru/Global/#mesh_test) 3/4 (с очисткой сопла). Если целевая температура сопла 230°C или ниже, перед замером сопло остывает до этой температуры. По умолчанию: `121`.

Уменьшите значение, если ваша пластина не выдерживает температуру по умолчанию: на пластинах PC (температура стеклования ~145-150°C) и некоторых G10 горячее сопло оставляет глубокие вмятины.

Этот параметр не задаётся через `SAVE_ZMOD_DATA` — введите команду в консоли Fluidd/Mainsail (значение сохраняется в `mod_data/variables.cfg`):

Пример: `SAVE_VARIABLE VARIABLE=temp_test_min VALUE=110`

*   Рекомендуемый диапазон: 100-150. Новое значение применяется со следующей печати, перезагрузка не требуется.
*   Используется только там, где выполняется очистка сопла ([MESH_TEST](/ru/Global/#mesh_test) 3/4). В режимах 1/2 (по умолчанию) карта проверяется на полной температуре печати без остывания.

---

##### TEMP_TEST_MAX

Верхняя температура сопла для замера карты стола в режимах [MESH_TEST](/ru/Global/#mesh_test) 3/4 (с очисткой сопла). Если целевая температура сопла выше 230°C, перед замером сопло остывает до этой температуры. По умолчанию: `151`.

Использование такое же, как у [TEMP_TEST_MIN](/ru/Global/#temp_test_min) — уменьшите значение для чувствительных к температуре пластин (PC, некоторые G10).

Пример: `SAVE_VARIABLE VARIABLE=temp_test_max VALUE=140`
...
Запретить очистку сопла линией 0-нет, 1-да (0)

[Подробнее](/ru/Global/#clear)
...
Выбрать алгоритм очистки сопла линией (LINE_PURGE) 
...
_Чтобы отключить очистку сопла линией используйте [DISABLE_PRIMING](/ru/Global/#disable_priming)_
...
- 0 - нет (включен датчик наличия филамента)
- 1 - да (включен датчик движения филамента)
...
На некоторых прошивках периодически не запускается Wi-Fi.

Чтобы это исправить, нужно подключиться к сети wifi через родной экран.

1. Вызывать `SAVE_ZMOD_DATA WIFI=1`
2. Выключить принтер
3. Включить принтер

- 0 использовать WiFi с родного экрана
- 1 использовать WiFi через Z-Mod
...
- Завис Nations MCU mainboard или eboard. `Lost communication with MCU 'mcu'`. Решение: Перезагрузка. Замена материнской платы(`mcu`) или платы экструдера('eboard').
...
- Нехватка оперативной памяти. Решение: перепайка процессора и увеличение размера памяти до 256 мегабайт
...
- Загрузка данных из SWAP (SWAP находится на EMMC, которая работает со скоростью 10 МБ/с, объем данных в SWAP при построении шейперов доходит до 25 мегабайт). Решение: отключение SWAP если у вас 256 мегабайт оперативной памяти `SAVE_ZMOD_DATA USE_SWAP=0`
...
* Большой объем исключений моделей: Решение `Профиль процесса` -> `Прочее` -> `Выходной G-code` -> `Исключение моделей` выключить галочку.
...
* Если используется Klipper 13 на AD5X. В этом случае можно вернуться на родной Klipper выполнив `UPDATE_MCU FORCE=12`
* Если используется fuzzy skin и процессор не успевает просчитать траекторию движения. Тогда нужно отключить fuzzy skin или подобрать его параметры.

Ошибка «Communication timeout during homing» может возникать из-за высокой задержки связи между хост-компьютером и микроконтроллерами. Обычно время в пути должно быть постоянно меньше 10 мс. Высокая задержка даже на короткие периоды может привести к сбоям при настройке.
...
Как исправить на стоке вручную:
...
Перенаправить экран принтера как виртуальную камеру `screen` во Fluidd/Mainsail и [zmod.link](https://zmod.link)

- 1 - Перенаправлять (по умолчанию)
- 2 - Не перенаправлять

Пример: `SAVE_ZMOD_DATA WEB_SCREEN=1`
...
- 0 - Загружает расположение кнопок макросов из Z-Mod (по умолчанию)
...
- 0 - Не сохранять состояние датчиков филамента после перезагрузки, они всегда будут включены (по умолчанию)
