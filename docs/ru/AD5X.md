...
Чтобы включить автоматическое сопоставление цветов из gcode-файла с физическими катушками, используйте параметр [AUTO_ASSIGN_COLORS](/ru/Global/#auto_assign_colors). Для работы этой функции необходимо активировать сканирование файлов. Использование значения 30 приведет к прерыванию печати в тихом режиме, если возникнут любые проблемы с автоматическим назначением. 
...
5.  **`filament_unload_into_tube`** — Сколько филамента извлекать из модуля 4 в 1, когда экструдер уже не используется. **По умолчанию: 70 мм.**
    ...
    *   Если у вас модуль 4 в 1 нового образца, то увеличьте `filament_unload_into_tube` или в крайнем случае увеличьте `nozzle_cleaning_length`
...
*   **`filament_ifs_speed`** — Скорость (в мм/м), с которой работает модуль IFS. **По умолчанию: 1200 мм/м (20 мм/с).**
...
Этот режим требует надлежащей поддержки со стороны профиля принтера в слайсере; в частности, необходим G-код смены филамента, который обрабатывает сброс пластика и последующий возврат к черновой башне. НЕ используйте этот режим с любым файлом G-кода, который не был специально нарезан под него.
...
## **7. Добавить или удалить типы филамента**
...
```
Где NEWTYPE заменить на нужный вам тип филамента (например HIPS), а число — это температура экструдера для загрузки, выгрузки и продувки этого филамента.
...
```
Где XXX, YYY, ZZZ заменить на нужные вам типы филамента (например PLA-CF,PETG-CF,SILK). Это можно использовать для скрытия типов по умолчанию или типов, которые вы добавили вручную с помощью настройки выше. При этом тип филамента не отключается, а только скрывается в меню выбора типа.
...
- `IFS_F18` - Отжим филамента везде
- `IFS_F23` - Помечаем пруток как вставленный
- `IFS_F24` - Прижим филамента
- `IFS_F39` - Отжим филамента
- `IFS_F112` - Прекращаем подачу прутка
...
- stall_count - с какой попытки засчитывать что пруток остановился (*1*)
- retry_count - сколько раз повторять команду при ошибке (*3*)
- next_cmd_delay - сколько ждать между обработкой ответа IFS и отправкой следующей команды (*0.2*)
- send_ff_terminator - включает или отключает задержку 0,2 с + отправку 0xFF после команды IFS; может потребоваться для очень старой прошивки IFS (True, *False*)
- filament_NEWFILAMENT - добавить новый тип филамента параметр - температура загрузки и выгрузки этого материала.
...
1. Соединяемся
2. Выбираем [файл для прошивки](/ru/Native_FW/#5x-ifs). **Не забудьте его разархивировать**.
