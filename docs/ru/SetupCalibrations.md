...
-  **Важно:**
    *   Принтер нагреет экструдер и стол до заданных температур.
    ...
    [Такая картинка появляется только на Klipper 13](https://wiki.zmod.link/ru/System/#update_mcu)
...
_Если меняете одну пластину, на другую (или стекло), то тоже нужно снимать карту стола, т.к. высота пластин может отличаться, при этом перекалибруется стартовый ноль под высоту новой пластины._
...
В большинстве случаев нет необходимости строить карту стола перед каждой печатью. Исключение — если вы используете **сменные пластины с разной толщиной** (например, PEI-лист и стекло), так как у них может быть разная высота, следовательно для печати будет разный стартовый ноль.
...
5.  Родной экран принтера сам сохранит введенное значение и будет использовать его при следующей печати
...
*   Регулировать Z-Offset через Fluidd/Mainsail при работе с родного экрана не следует, т.к. родной экран не знает о существовании Fluidd/Mainsail
...
**Особенность для принтеров с малым объемом памяти (AD5M, AD5M Pro):**
