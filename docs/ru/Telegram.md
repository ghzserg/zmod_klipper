...
Учитывая, что в последнее время наблюдаются сбои в работе Telegram - есть альтернативное решение - создание таймлапсов на отдельном сервере, оторый использовался для телеграм бота. 
Пример содержится в репозитории [https://github.com/seligor/printer_timelapse](https://github.com/seligor/printer_timelapse)
С минимальными доработками его можно совместить с [https://github.com/seligor/web-timelapse](https://github.com/seligor/web-timelapse) и получить аналогичный функционал, но без использования мессенджера. 

Основной функционал: опрос принтера на предмет начавшейся печати. При начале печати - сбор скриншотов. По окончании печати - генерация видео из сккриншотов, создание кадра превью. Поддерживает несколько принтеров. 
