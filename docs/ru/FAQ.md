...
> Поставили мод.
> 
> Не хотите ни в чём разбираться - печатайте как и печатали.
> 
> Вообще ничего нигде настраивать и менять не надо.
> 
> Решили, что готовы двигаться дальше - двигаетесь - читая документацию.

### Чем Z-Mod отличается от KlipperMod/родной прошивки

   Разница между KlipperMod и Z-Mod:

  - KlipperMod использует чистый klipper с минимумом специфичных для flashforge 5m(pro) изменений
      - Z-Mod использует стандартный Klipper из родной прошивки, а также Klipper 13
      - KlipperMod использует KlipperScreen как экран для принтера.
      - В Z-Mod вместо KlipperScreen родной экран или GuppyScreen/HelixScreen
      - KlipperMod использует Moonraker-timelapse
      - Z-Mod использует moonraker-telegram-bot на ВНЕШНЕМ хосте с поддержкой таймлапс или [плагин TimeLapse](https://github.com/ghzserg/timelapse/)

Разная философия.

* KlipperMod - по сути альтернативная реализация прошивки.
* Z-Mod минимальное вмешательство в родную прошивку. Все функции родной прошивки сохранены.

Именно поэтому в Z-Mod не будет G17, G18, G19 - хотя это просто. Не будет обновлений родного клиппера, не будет переименований или изменений стандартных макросов, настроек, названия пинов и т.д.

Z-Mod НЕ основан на KlipperMod, и НЕ является его развитием. При этом в Z-Mod используются некоторые макросы и скрипты из KlipperMod, а также использовались наработки из KlipperMod. Но не стоит ожидать от Z-Mod поведения аналогичного KlipperMod.

**Z-Mod бинарно не совместим с KlipperMod.**

#### Что есть в KlipperMod и чего нет в Z-Mod:

- [KlipperScreen](https://klipperscreen.readthedocs.io/en/latest/) - экран для принтера. В Z-Mod вместо KlipperScreen родной экран или GuppyScreen/HelixScreen
- [Moonraker-timelapse](https://github.com/mainsail-crew/moonraker-timelapse) - в Z-Mod используется Телеграм бот или [плагин TimeLapse](https://github.com/ghzserg/timelapse/)
- Настройка сети через iwd/wpa_supplicant (в случае с GuppyScreen/HelixScreen) - в Z-Mod настройка сети через родной экран, запуск сети возможен и в режиме без родного экрана

#### Что есть в Z-Mod и чего нет в KlipperMod:
...
- Автоматическое обновление `Fluidd`/`Mainsail`/`Moonraker` и Z-Mod по сети
...
#### Что есть в Z-Mod и чего нет в родной прошивке:
...
- [Родная прошивка отправляет множество данных на китайские сервера](https://github.com/FlashForge/Orca-Flashforge/issues/26), этого можно избежать, если использовать Z-Mod с GuppyScreen/HelixScreen
...
- Пользовательские настройки klipper необходимо вносить в файл `mod_data/user.cfg`, настройки записанные в это файл могут заменять/дополнять настройки из `printer_base.cfg` и файлов Z-Mod.
...
- Если принтер выполняет какие-то действия `M109` (прогрев экструдера), `M190`(прогрев стола), калибровку PID - в общем любое действие, которое заставляет приостановиться gcode, то родной экран замирает
- Если перезапускается klipper (после сохранения карты стола, PID, Шейперов и т.д.), то родной экран замерзает (используйте перезапуск через [NEW_SAVE_CONFIG](/ru/Main/#new_save_config) или [глобальный параметр](/ru/Global/#new_save_config)
- После отмены печати, на родном экране надо нажать кнопку Ок (используйте макрос [CLOSE_DIALOGS](/ru/Main/#close_dialogs) или [FAST_CLOSE_DIALOGS](https://github.com/ghzserg/zmod/wiki/Main_ru#fast_close_dialogs) или [глобальный параметр](/ru/Global/#close_dialogs))
...
- Не работает отправка через протокол FlashForge из Orca, т.к. его обрабатывает родной экран.
...
Принтер может работать в двух режимах:

- С родным экраном - в этом случае практически вся логика работы управляется родным экраном и многие вещи изменить нельзя.
- Без родного экрана - в этом случае всеми возможностями управляет Z-Mod.

Это не значит что вам нужно аппаратно отключать экран или менять его на другой.
В режиме без родного экрана вы можете использовать альтернативный программный экран GuppyScreen/HelixScreen или вообще отключить экран и он потухнет.

**Не отключайте экран, если вы четко не понимаете как работает карта стола, z-offset и макросы START_PRINT и END_PRINT**

В AD5M/AD5M Pro 128 мегабайт памяти, половину ест система, а 13 мегабайт (20 в старых версиях родной прошивки) съедает управление родным экраном.
...
```START_PRINT EXTRUDER_TEMP= BED_TEMP=``` должно писаться в одну строку без пробелов после =
...
Прочитайте документацию к [START_PRINT](/ru/Main/#start_print) и [SAVE_ZMOD_DATA](/ru/Global/#save_zmod_data), это позволит вам использовать расширенные и полезные функции Z-Mod
...
Смещение Z из Fluidd/Mainsail/GuppyScreen/HelixScreen влияет **только до перезагрузки** и менять его без понимания, куда движется сопло не стоит.

Любой вызов `SET_GCODE_OFFSET` (а он автоматически вызывается при изменении смещения Z из Fluidd/Mainsail/GuppyScreen/HelixScreen) сохраняет текущий z-offset в глобальные параметры мода. Но это сохраненное значение используется только в том случае, если указан глобальный параметр [LOAD_ZOFFSET](/ru/Global/#load_zoffset)(который по умолчанию отключен, для включения `SAVE_ZMOD_DATA LOAD_ZOFFSET=1`), не используется родной экран и используется макрос [START_PRINT](/ru/Main/#start_print).
...
- или загрузить карту стола в любом удобном месте (например в профиле прутка) через команду 
  ...
  Убедитесь что у вас используется одна и та же карта в профиле прутка и `START_PRINT`, или отключите очистку сопла в `START_PRINT`, запуская ее через профиль прутка.
...
- Если работаете в режиме без родного экрана и используете макрос [START_PRINT](/ru/Main/#start_print) в начальном G-коде, то при каждой печати тоже будет сниматься карта стола
  ...
  ```SAVE_ZMOD_DATA PRINT_LEVELING=1``` *(нужно вводить в консоль Fluidd/Mainsail)*. В этом случае карта стола будет сниматься при каждой печати.
...
- Можно включить адаптивное снятие карты стола (KAMP), тогда будет сниматься не весь стол, а только те части, где есть модели для печати. 
  **Автоматическое снятие карты стола не запускается!**. Этот параметр говорит, что если вызывается снятие карты стола, то запускать вместо него KAMP.
  ...
  ```SAVE_ZMOD_DATA USE_KAMP=1``` *(нужно вводить в консоль Fluidd/Mainsail)*. В этом случае, будет использоваться адаптивная карта стола везде, где возможно, в том числе и при снятии карты стола родным экраном по сети. 
...
3. В противном случае, если стоит `FORCE_KAMP=True`, то снимается KAMP
...
Если вы загружаете карту, через команды KLIPPER в профиле прутка, убедитесь, что у вас используется одна и та же карта в `START_PRINT` и профиле прутка, или отключайте очистку сопла в START_PRINT и запускайте ее отдельно через профиль прутка.
...
### Я хочу удалить Z-Mod - мне придется все перекалибровать?
...
Прочитайте: [Я установил принтер, а Z-Mod спрятал мою камеру!](#я-установил-принтер-а-zmod-спрятал-мою-камеру-в-orca-ff-я-ее-видел-а-теперь-она-пропала)
...
### Я установил принтер, а Z-Mod спрятал мою камеру! В Orca-FF я ее видел, а теперь она пропала!
...
_Если вы считаете, что эта ошибка появилась у вас во время печати, значит у вас короткое замыкание нагревателя (после долгой работы) и принтер перезагружается, но вы этого не замечаете._
...
Отличие `REBOOT` от `FIRMWARE_RESTART` в том, что `REBOOT` перезагружает linux, а с ним и Klipper на материнской плате, `FIRMWARE_RESTART` перезапускает klipper и полностью перезапускает MCU
...
Итак, какие порты использует Z-Mod:
...
### В Z-Mod входит Entware - как им воспользоваться?
...
В Z-Mod во Fluidd/Mainsail есть ползунки управления скоростью и величиной отката из прошивки.

![](../images/retract.jpg)

Они не влияют на печать и не отображают настройки ретракта из Orca, если g-code файл нарезан без использования параметров отката из прошивки.
...
Макрос `SET_RETRACTION` обычно устанавливается как часть конфигурации слайсера для каждого филамента, так как для разных филаментов требуются разные настройки параметров:
`SET_RETRACTION [RETRACT_LENGTH=<мм>] [RETRACT_SPEED=<мм/с>] [UNRETRACT_EXTRA_LENGTH=<мм>] [UNRETRACT_SPEED=<мм/с>]`:
...
Программно это правится вот так:
...
WeightValue - это значение на тензодатчиках в граммах. Оно отображается в градусах, т.к реализовано через интерфейс термодатчика. Поэтому Fluidd и Mainsail отображают градусы.

Зачем нужен этот датчик?

- С его помощью можно замерять zoffset через плагин [g28_tenz](https://github.com/ghzserg/g28_tenz)
- Можно прекращать печать, если сопло ударилось о деталь или деталь была оторвана. [NOZZLE_CONTROL](/ru/Global/#nozzle_control)
...
Суть всех этих ошибок в том, что версия Klipper не соответствует версии MCU.
...
Если у вас версия MCU начинается с ```?-20230317_182329-ubuntu20-virtual-machine``` - значит загружен MCU для Klipper 12 (AD5X) или Klipper 11 (AD5M/AD5M Pro).

[Перейти на родной Klipper](/ru/Native_FW/#fix_klipper)

Соответственно, нужно, чтобы Z-Mod загрузил родной Klipper.
...
Для AD5M нужно подбором откалибровать шаги датчика. Пропишите в `mod_data/user.cfg`
...
Для AD5X нужно подбором откалибровать шаги датчика IFS. Пропишите в `mod_data/user.cfg`
