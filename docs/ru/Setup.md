...
3. Поместить файл из [Native firmware](/ru/Native_FW/) в корневую папку USB Flash
    ...
    - [AD5X-1.1.7-1.1.0-3.0.6-20250912.tgz](https://github.com/ghzserg/FF/releases/download/R/AD5X-1.1.7-1.1.0-3.0.6-20250912-Factory.tgz) для AD5X
...
Если мод пишет `Обновите Z-Mod с флешки`, то нужно обновить Z-Mod с флешки, даже если вы его недавно обновляли.
...
- Далее переходите во Fluidd/Mainsail на вкладку `Настройки` -> `Обновление ПО`. Нажмите `Проверить обновления` и установите последние обновления `Z-Mod`
...
Текущая версия Z-Mod (вкладка "Настройки" -> "Обновление" -> "ffm5/zmod"), **должна совпадать** первыми двумя цифрами с версией со вкладки система.

<u>Если не совпадает, то мод **будет работать некорректно**, в этом случае не надо жаловаться на Z-Mod</u>
...
11. Включить принтер
12. Переходите во Fluidd/Mainsail на вкладку `Настройки` -> `Обновление ПО`. Нажмите `Проверить обновления` и установите последние обновления `Z-Mod`
...
- [SKIP_Z-Mod](/ru/Zmod/#skip_zmod) - макрос перезагрузки без запуска moonraker и fluidd
- [REMOVE_Z-Mod](/ru/Zmod/#remove_zmod) - макрос удаления мода

Рекомендуется **удалять мод через макрос `REMOVE_Z-Mod`**, удаление через флешку использовать, только если нет возможности запустить макрос.
...
Полностью удалить мод ```REMOVE_Z-Mod FULL=1```
...
3. Включите китайские облака, если хотите обновиться с родного экрана ```SAVE_Z-Mod_DATA CHINA_CLOUD=1```
