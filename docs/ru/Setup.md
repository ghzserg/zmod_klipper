...
Рекомендуется **удалять мод через макрос `REMOVE_ZMOD`**, удаление через флешку использовать, только если нет возможности запустить макрос (_после удалиения через флешку может остаться логотип Z-Mod_).
